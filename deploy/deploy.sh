...
# Kong is DB-less and reads kong.yml at start. `kong reload` proved unreliable
# for picking up file changes, so force-recreate the container (deterministic).
log "Recreating Kong to load latest declarative config..."
docker compose "${COMPOSE_FILES[@]}" --env-file "$ENV_FILE" \
  up -d --force-recreate kong >/dev/null 2>&1 && log "kong recreated" || log "WARN: kong recreate failed"
